...
class Moderation(commands.Cog, name="moderation-normal"):
    ...
    async def kick(self, context: Context, member: disnake.Member, *, reason: str = "Not specified") -> None:
        ...
        if member.guild_permissions.administrator:
            ...
            try:
                ...
                logger.info(f"{member} was kicked by {context.author} for {reason}!")
                await context.send(embed=embed)
                try:
                    await member.send(
    ...
    async def nick(self, context: Context, member: disnake.Member, *, nickname: str = None) -> None:
        ...
        try:
            ...
            logger.info(f"{member}'s nickname was changed to {nickname} by {context.author}!")
            await context.send(embed=embed)
        except:
            embed = disnake.Embed(
                title="Error!",
    ...
    async def ban(self, context: Context, member: disnake.Member, *, reason: str = "Not specified") -> None:
        ...
        try:
            if member.guild_permissions.administrator:
                ...
                logger.info(f"{member} was banned by {context.author} for {reason}!")
                await context.send(embed=embed)
                try:
    ...
    async def warn(self, context: Context, member: disnake.Member, *, reason: str = "Not specified") -> None:
        ...
        logger.info(f"{member} was warned by {context.author} for {reason}!")
        await context.send(embed=embed)
        try:
    ...
    async def purge(self, context: Context, amount: int) -> None:
        ...
        logger.info(f"{context.author} cleared {len(purged_messages)} messages!")
        await context.send(embed=embed)
    ...
    async def hackban(self, context: Context, user_id: int, *, reason: str) -> None:
        ...
        try:
            ...
            logger.info(f"{user} (ID: {user_id}) was banned by {context.author} for {reason}!")
            await context.send(embed=embed)
        except:
            embed = disnake.Embed(
                title="Error!",
    ...
    async def newrole(self, context: Context, *, name: str) -> None:
        ...
        try:
            ...
            logger.info(f"{name} was created by {context.author}!")
            await context.send(embed=embed)
        except:
            embed = disnake.Embed(
                title="Error!",
